...
import WomenStory from '../../../src/components/pages/women/womenStory'
...
import PictureSlider from '../../../src/components/pages/women/PictureSlider'
...
export default function PersonalPage({ woman, suggestWoman }) {
  ...
  const download = async (values) => {
    ...
    setLoader(true)
    values.formSubject = 'Լրացուցիչ տեղեկություններ'
    values.women = `${last_name} ${first_name}`
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      email: Yup.string().email('Չի համապատասխանում ձևաչափին'),
    }),
  })
    ...
    <>
      ...
      <MainLayout>
        <WomanPageHeroBanner
          ...
          imageHeight={PageConstants.imgSizes[avatarSize]}
        />
        <div
          className={
          ...
          <div className={'col-span-4 lg:col-span-8'}>
            {women_stories.data.length
              ? women_stories?.data?.map((story) => {
                  ...
                  return (
                    <WomenStory title={name} text={text} id={name} key={name} />
                  )
                })
              : null}

            {images?.data?.length ? (
              <Images images={images.data} setSliderToggle={setSliderToggle} />
            ) : null}
      ...
      {sliderToggle ? (
        <PictureSlider images={images.data} setSliderToggle={setSliderToggle} />
      ) : null}
    </>
  )
}
